...
func (rm *resourceManager) ResolveReferences(
	...
	if fieldHasReferences, err := rm.resolveReferenceForAuthorizerCredentialsARN(ctx, apiReader, ko); err != nil {
		return &resource{ko}, (resourceHasReferences || fieldHasReferences), err
	} else {
		resourceHasReferences = resourceHasReferences || fieldHasReferences
	}
...
func validateReferenceFields(ko *svcapitypes.Authorizer) error {
	...
	if ko.Spec.AuthorizerCredentialsRef != nil && ko.Spec.AuthorizerCredentialsARN != nil {
		return ackerr.ResourceReferenceAndIDNotSupportedFor("AuthorizerCredentialsARN", "AuthorizerCredentialsRef")
	}
	return nil
}
...
// resolveReferenceForAuthorizerCredentialsARN reads the resource referenced
// from AuthorizerCredentialsRef field and sets the AuthorizerCredentialsARN
// from referenced resource. Returns a boolean indicating whether a reference
// contains references, or an error
func (rm *resourceManager) resolveReferenceForAuthorizerCredentialsARN(
	ctx context.Context,
	apiReader client.Reader,
	ko *svcapitypes.Authorizer,
) (hasReferences bool, err error) {
	if ko.Spec.AuthorizerCredentialsRef != nil && ko.Spec.AuthorizerCredentialsRef.From != nil {
		...
		ko.Spec.AuthorizerCredentialsARN = (*string)(obj.Status.ACKResourceMetadata.ARN)
	}
